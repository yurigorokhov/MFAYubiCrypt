...
\todo{add parts from the conclusion}
...
\todo{Add that this system ought to be used on sensitive data. Survalliance society + concrete facts and data to support this claim.}
...
\todo{rewrite following to reflect new structure -- minus Hereafter, lastly}
...
The proposed system is composed of serveral user clients, hard drive client and a verification server. The user clients forms a quorum that can respond to requests from hard drive clients to unlock files. The basic idea is for server and user clients to be instantiated with a shared secret, see figure ~\ref{fig:basicSystemFigure}.

This structure allows for a distribution of the key that is needed to unlock the encrypted file in question on the hard drive client. This is ensured by having the server encrypt the key used to encrypt the actual file, with every single users information (login credential, Yubikey secret and sequence number). The actual file encryption key are reffered to as the Disk Encryptio Key (DEK). 
...
Compromising the server would not get the attacker access to the decryption key, as it only stores the encrypted values of each clients secrets. An attacker would need to compromise the server and the clients in order to get all the information needed to decrypt the indivual keys on the server side, such that the DEK is revealed. 
...
Yubikey is a marketed USB dongle used for various Multifactor authentication purposes. It can be set up in different modes, for One Time Password (OTP) based on a series of variables, including sequence numbers. In this work we used the Challenge-Response mode, where the Yubikey is configured with a shared Secret Key among the server and the Yubikey itself. The Secret Key is a SHA-1 cryptographic hash function. 
...
The hard drive client (the client program that is in charge of decrypting the drive) generates a DEK to encrypt the hard drive with. Once encrypted, a request is sent to the server to set up a quorum of users with to participate in the encryption. The server responds with an encryptionId and a secret cryptographic hash \( SHA(\sum key) \). This hash, along with the DEK are AES-encrypted and stored on the hard drive client. The key used for the encryption is calculated as follows:
\begin{equation}
...
\subsection{Hard drive client attacks}\hspace*{\fill} \\
