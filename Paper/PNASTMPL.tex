...
\begin{abstract}
In some file access schemes, it would be beneficial to only be able to decrypt a given file with the consent of a quorum. This paper presents a system for securely providing that functionality. Distributing this functionalty over a server and multiple clients, allowing for remote consent from the formed quorum for a given file. Once the clients and server are initialized with shared secrets, no information is sent over the wire unencrypted.
