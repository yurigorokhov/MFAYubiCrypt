...
The two most noticable attack vectors identified, are when keys are held in-memory and the point of distribution of the secrets to clients.
...
\dropcap{P}rotecting information on a computer storage device against unauthorized access, even when the storage device or the computer itself is stolen or compromised by an attacker is a challeging task.
