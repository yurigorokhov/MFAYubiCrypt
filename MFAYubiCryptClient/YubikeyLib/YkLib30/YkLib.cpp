...
/*************************************************************************
**  function writeNDEFBegin                                             **
**  Start a NDEF record write                                           **
**                                                                      **
**  YKLIB_RC writeNDEFBegin(unsigned char slot, YKLIB_NDEF_TYPE type,   **
**                          char *ndefStr, unsigned char *curAccCode)   ** 
**                                                                      **
**  Where:                                                              **
**  "slot" is slot number to associate the NDEF to 0..                  **
**  "type" is NDEF type                                                 **
**  "ndefStr" is the string to write. NULL kills NDEF association       **
**  "curAccCode" is pointer to current access password (if any)         **
**                                                                      **
**  Returns: Status return code                                         **
**                                                                      **
...
YKLIB_RC CYkLib::writeNDEFBegin(unsigned char slot, YKLIB_NDEF_TYPE type, char *ndefStr, unsigned char *curAccCode)
{
    YKLIB_RC rc;
    YKNDEF ndef;
    static const char *ids[] = {
        NDEF_URI_ID1, NDEF_URI_ID2, NDEF_URI_ID3, NDEF_URI_ID4, NDEF_URI_ID5,
        NDEF_URI_ID6, NDEF_URI_ID7, NDEF_URI_ID8, NDEF_URI_ID9, 0
    };

    memset(&ndef, 0, sizeof(ndef));

    // If NDEF string is present, parse it. Otherwise, just overwrite (kill) the NDEF record

    if (ndefStr) {
        switch (type) {
            case YKLIB_NDEF_URI:
                for (ndef.data[0] = 0; ids[ndef.data[0]]; ndef.data[0]++)
                    if (!strncmp(ndefStr, ids[ndef.data[0]], strlen(ids[ndef.data[0]]))) break;
                if (ids[ndef.data[0]]) {
                    ndefStr += strlen(ids[ndef.data[0]]);
                    ndef.data[0]++;
                }
                ndef.type = NDEF_URI_TYPE;
                break;
            case YKLIB_NDEF_TEXT:
                ndef.type = NDEF_TEXT_TYPE;
                break;
            default:
                return YKLIB_INVALID_PARAMETER;
        }

        // Copy the string and make sure it fits

        for (ndef.len = 1; *ndefStr; ndef.len++) {
            if (ndef.len >= sizeof(ndef.data)) return YKLIB_INVALID_PARAMETER;
            ndef.data[ndef.len] = *ndefStr++;
        }
    }

    // Insert access code if present

    if (curAccCode) memcpy(ndef.curAccCode, curAccCode, sizeof(ndef.curAccCode));

    // Start write request

    rc = rawWrite(slot ? SLOT_NDEF2 : SLOT_NDEF, (BYTE *) &ndef, sizeof(ndef));
